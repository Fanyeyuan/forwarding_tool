...
class XPH{
    ...
    getRealData(id) {
        return axios.get(`${this.baseUrl}/intfa/queryData/${id}`)
        .then((res)=>{
    ...
    getDevices(){
        ...
        return axios.get(`${this.baseUrl}/user/${this.userInfo.username}`,
            {
                headers: {
        ...
        .then((res) => {
            ...
            if(info.devices){
                this.devices = info.devices.map((device) => {
                    return {id: device.facId, name:device.facName, pk: device.remark}
                })
    ...
    sample(username, password){
        if(!username || !password){
            console.log(`请输入正确的用户名和密码，当前用户名-${username}/密码-${password}`)
            return false
        }
